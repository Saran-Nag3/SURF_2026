...
load_dotenv()  # so .env is read when running Phase 2 directly (Azure vars, DESIGN_DIR)
...
def synthesize(g: dict, company: str, annotations: Optional[dict] = None) -> str:
    """Render one company's guidelines into the canonical DESIGN.md format.

    Always emits every section in SECTION_ORDER. `annotations` (optional, from
    llm_synth) may override color roles/names, add per-color and per-component
    purpose notes, and supply the Overview prose - but only for values that exist
    in the extraction; ungrounded entries are dropped upstream. Without it the
    output is fully deterministic.
    """
    annotations = annotations or {}
    ann_colors = annotations.get("colors") or {}
    ann_comps = annotations.get("components") or {}
    overview = (annotations.get("overview") or "").strip()
    ...
    # Apply grounded LLM annotations: role/name overrides + purpose notes.
    purpose_by_hex = {}
    for s in swatches:
        a = ann_colors.get(s.hex)
        if not a:
            continue
        if a.get("role"):
            s.role = a["role"]
        if a.get("name"):
            s.name = a["name"]
        if a.get("purpose"):
            purpose_by_hex[s.hex] = a["purpose"]
    if ann_comps:
        components = [
            (f"{ln} — {ann_comps[ln.split(':', 1)[0].strip()]['purpose']}"
             if ann_comps.get(ln.split(":", 1)[0].strip(), {}).get("purpose") else ln)
    ...
    def swatch_line(s: Swatch):
        ...
        tail = f" — {purpose_by_hex[s.hex]}" if s.hex in purpose_by_hex else ""
        return f"`{s.hex}` - {s.role}: {label}{use}{tail}"
...
# --- optional LLM authoring pass --------------------------------------------
def author_with_llm(files: list, merged: dict, company: str,
                    raw_dump_path: Path, deployment: Optional[str]) -> Optional[str]:
    """Let the LLM author the whole DESIGN.md from the raw dump. None -> fall back."""
    try:
        import llm_synth
    except Exception as e:
        log.warning("%s: llm_synth import failed (%s); deterministic path", company, e)
        return None
    if not llm_synth.available():
        log.warning("%s: --llm set but Azure OpenAI not configured "
                    "(set AZURE_OPENAI_KEY); deterministic path", company)
        return None
    if raw_dump_path.exists():
        raw = raw_dump_path.read_text(encoding="utf-8")
    else:
        raw = json.dumps(files, indent=2)  # fall back to the structured dump
    try:
        md = llm_synth.author(raw, files, company, SECTION_ORDER, deployment=deployment)
        log.info("%s: DESIGN.md authored by LLM", company)
        return md
    except Exception as e:
        log.warning("%s: LLM authoring failed (%s); deterministic path", company, e)
        return None


# --- orchestration ----------------------------------------------------------
def run(company_filter: Optional[str], use_llm: bool, deployment: Optional[str]) -> None:
    ...
    for d in dirs:
        ...
        md = None
        if use_llm:
            md = author_with_llm(files, merged, d.name, d / "raw_value_dump.md", deployment)
        if md is None:                        # default, or LLM unavailable/failed
            md = synthesize(merged, d.name)

        (d / "DESIGN.md").write_text(md, encoding="utf-8")
        log.info("%s: DESIGN.md written (%d chars)", d.name, len(md))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--llm", action="store_true",
                    help="grounded Azure OpenAI annotation (roles/names/purposes/overview)")
    ap.add_argument("--deployment", default=None,
                    help="Azure deployment name (default: llm_synth config)")
    ...
    run(args.company, args.llm, args.deployment)
