...
# Config
# --------------------------------------------------------------------------
INPUT_FILE   = "final_no_ar.csv"
OUTPUT_DIR   = "no_ar_svg"
...
def top_level_frames(file_data):
    """Return [(node_id, name), ...] for top-level 'screens' across pages.

    Matches Stage 2's generous definition so files that PASSED enrichment don't
    export nothing here: top-level FRAME / COMPONENT / COMPONENT_SET / INSTANCE
    count directly, and we descend one level into wrapper SECTION / GROUP nodes.
    (Previously this only collected FRAME, so component-based files — design
    systems, icon/component libraries — exported zero screens and their company
